...
// Per caricare la mesh dal file .obj e anche 
function loadMesh(mesh) {
    ...
    for(var j = 0; j < mesh.materials.length; j++) {
        ...
        // Aggiunta
        // Assegno la texture al mio modello
        ...
        for (let i = 1; i <= nface; i++) {
            ...
            mesh.positionsLoaded.push(x[i0], y[i0], z[i0], x[i1], y[i1], z[i1], x[i2], y[i2], z[i2]); // Aggiunta

            i0 = mesh.data.facetnorms[i].i;
            i1 = mesh.data.facetnorms[i].j;
            i2 = mesh.data.facetnorms[i].k;
            mesh.normalsLoaded.push(i0, i1, i2, i0, i1, i2, i0, i1, i2); // Aggiunta

            i0 = mesh.data.face[i].textCoordsIndex[0] - 1;
            i1 = mesh.data.face[i].textCoordsIndex[1] - 1;
            i2 = mesh.data.face[i].textCoordsIndex[2] - 1;
            mesh.texcoordsLoaded.push(xt[i0], yt[i0], xt[i1], yt[i1], xt[i2], yt[i2]); // Aggiunta
        }

        // Aggiunta
        // Assegno il numero delle facce del modello
